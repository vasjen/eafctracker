...
namespace Eafctracker.Services
{
    public class ProfitService :IProfitService
    {
        ...
        public ProfitService(ISalesHistoryService history, IPriceService priceService, ApplicationDbContext context, ITelegramService tggbot, IHttpClientService client)
        {
            _history=history;
                ...
                if (profit> 0 && profit>=_minProfit){  
                    ...
                    var avgPrice=(lastTenSales!.OrderByDescending(p=>p.Price).Select(p=>p.Price).Sum())/10;
                      if (NextPrice<=avgPrice)  
                      {
                        ...
                        System.Console.WriteLine($"Profit: {profit} for {card.Name} {card.Revision}");
