...
namespace Eafctracker.Services
{
    public class UpdateService : IUpdateService
    {
        ...
        public UpdateService(ApplicationDbContext context, IHttpClientService service, IInitialService initial)
        {
            _context=context;
            _service=service;
            _initial=initial;
            ...
            if (maxFbIdFromSite>maxFbIdFromDb & maxFbIdFromSite>maxFbIdFromLastUpdate)
              {
                ...
                System.Console.WriteLine("Trying to get a new cards");
                  ...
                  foreach (var card in NewCards){
                    System.Console.WriteLine($"Had add card with name: {card.Name} {card.Revision} => FbId: {card.FbId}, FbDataId: {card.FbDataId}, Tradable? {card.Tradable}");
                  }
              }
              else {
